...
class IPsecVpnDriverApi(object):
    ...
    def get_vpn_services_on_host(self, context, host,vpntype=None):
        """Get list of openvpn.

        The openvpn including related ipsec_site_connection,
        ikepolicy and ipsecpolicy on this host
        """
        cctxt = self.client.prepare()
        return cctxt.call(context, 'get_vpn_services_on_host', host=host,vpntype=vpntype)
    ...
    def update_status(self, context, status,vpntype=None):
        ...
        """
        cctxt = self.client.prepare()
        return cctxt.call(context, 'update_status', status=status,vpntype=vpntype)
...
class IPsecDriver(device_drivers.DeviceDriver):
    ...
    def report_status(self, context):
        ...
        if status_changed_vpn_services:
            self.agent_rpc.update_status(
                context,
                status_changed_vpn_services,vpn_consts.IPSEC)
    ...
    def sync(self, context, routers):
        ...
        vpnservices = self.agent_rpc.get_vpn_services_on_host(
            context, self.host,vpn_consts.IPSEC)
