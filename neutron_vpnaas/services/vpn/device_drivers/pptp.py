...
pptp_opts = [
    cfg.StrOpt(
        'config_base_dir',
        default='$state_path/pptpd',
        help=_('Location to store pptp server config files')),
    cfg.IntOpt(
        'status_check_interval',
        default=60,
        help=_("Interval for checking pptp status")),
    cfg.StrOpt(
        'config_template',
        default=os.path.join(
            TEMPLATE_PATH,
            'template/pptpd/pptpd.conf.template'),
        help=_('Template file for pptpd configuration')),
    cfg.StrOpt(
        'options_template',
        default=os.path.join(
            TEMPLATE_PATH,
            'template/pptpd/options.pptpd.template'),
        help=_('Template file for pptpd options configuration')),
    cfg.ListOpt(
        'dns',
        default=['8.8.8.8', '4.4.4.4'],
        help=_('DNS for pptpd server.')),
]
...
class BasePPTPDProcess(linux_driver.BaseLinuxProcess):
    """PPTP Family Process Manager
    ...
    def __init__(self, conf, process_id, vpnservice, namespace):
        ...
        config_dir = os.path.join(
            cfg.CONF.pptp.config_base_dir, process_id)
        super(BasePPTPDProcess, self).__init__(conf, process_id,
...
class PPTPDProcess(BasePPTPDProcess):
    """PPTPD Process manager class.

    This process class uses two commands
    (1) pptpd:  pptpd daemon
    (2) pppstats: show ppp status
    """
    def __init__(self, conf, process_id, vpnservice, namespace):
        super(PPTPDProcess, self).__init__(conf, process_id,
                                           vpnservice, namespace)
    ...
    def _user_modify(self, cmd):
        #TODO: There are race conditions when updating the chap-secrets file
        utils.execute(['sed', '-i', cmd, '/etc/ppp/chap-secrets'],
                      run_as_root=True)

    def update_user(self, vpnservice_id, credentials):
        for cred in credentials:
            # Delete the user first.
            deluser = ('/%(user)s %(service_id)s/ d' %
                       {'user': cred['username'],
                        'service_id': vpnservice_id})
            self._user_modify(deluser)
            # Then add the user.
            adduser = ('a %(user)s %(service_id)s %(password)s *' %
                       {'user': cred['username'],
                        'service_id': vpnservice_id,
                        'password': cred['password']})
            self._user_modify(adduser)

    def delete_user(self, vpnservice_id, credentials):
        for cred in credentials:
            deluser = ('/%(user)s %(service_id)s/ d' %
                       {'user': cred['username'],
                        'service_id': vpnservice_id})
    ...
    def remove_config(self):
        ...
        if self.vpnservice and self.vpnservice.get('pptpconnections'):
            ...
            self.delete_user(self.vpnservice['id'], creds)
    ...
    def ensure_configs(self):
        ...
        # Only support one pptpd deamon in one vpnservice
        ...
        self.ensure_config_file(
            'pptpd.conf',
            self.conf.pptp.config_template,
            pptpd_conf_data)
        options_data = {'id': self.vpnservice['id'],
                        'dnss': self.conf.pptp.dns}
        self.ensure_config_file(
            'options.pptpd',
            self.conf.pptp.options_template,
            options_data)
        creds = self.vpnservice['pptpconnections'][0]['credentials']
        self.update_user(self.vpnservice['id'], creds)
    ...
    def start(self):
        ...
        if not self.namespace:
            return
        config_file = self._get_config_filename('pptpd.conf')
        options_file = self._get_config_filename('options.pptpd')
...
class PPPDriver(device_drivers.DeviceDriver):
    ...
    def _sync_vpn_processes(self, vpnservices, sync_router_ids,
        ...
        for vpnservice in vpnservices:
            if vpnservice['router_id'] not in self.processes or (
                ...
                #     process.disable()
                if credential and credential.get('action') == 'delete':
                    process.delete_user(vpnservice['id'], [credential])
                elif credential and credential.get('action') == 'update':
                    process.update_user(vpnservice['id'], [credential])
                else:
                    process.update()

    def _delete_vpn_processes(self, sync_router_ids, vpn_router_ids):
        # Delete any PPTP processes that are
    ...
    def _cleanup_stale_vpn_processes(self, vpn_router_ids):
        # Delete any PPTP processes running
