...
from neutron_vpnaas.services.vpn.common import constants as vpn_constants
...
class VPNPluginDb(vpnaas.VPNPluginBase, base_db.CommonDbMixin):
    ...
    def _get_resource(self, context, model, v_id):
        try:
            ...
            with excutils.save_and_reraise_exception(reraise=False) as ctx:
                ...
                elif issubclass(model, vpn_models.OpenvpnConnection):
                    raise openvpn.OpenvpnConnectionNotFound(id=v_id)
                elif issubclass(model, vpn_models.PPTPConnection):
                    raise pptp.PPTPConnectionNotFound(conn_id=v_id)
                ctx.reraise = True
        return r
    ...
    def _make_vpnservice_dict(self, vpnservice, fields=None):
        res = {'id': vpnservice['id'],
               ...
               'service_type': vpnservice['service_type'],
...
class VPNPluginRpcDbMixin(object):
    ...
    def update_ipsec_status_by_agent(self, context, service_status_info_list):
